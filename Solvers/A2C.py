...
def actor_loss():
    def loss(advantage, predicted_output):
        ...
        ################################
        b = advantage
        gradient_loss = K.sum(-b * K.log(predicted_output))
...
def critic_loss():
    def loss(advantage, predicted_output):
        ...
        ################################
        b = advantage
        gradient_loss  = K.sum(K.square(b-predicted_output))
        return gradient_loss
        raise NotImplementedError
...
class A2C(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            random_action = np.random.choice(np.arange(nA), p=self.policy(state))
            next_state, reward, done, _ = self.step(random_action)
            states.append(state)
            actions.append(random_action)
            critic_target = reward
            t = self.actor_critic.predict([[next_state]])[1][0][0]
            if done==False:
                critic_target+= self.options.gamma * t
            delta = critic_target - t
            deltas.append([delta])
            if done == True:
                break
            state = next_state
